...
public class ScrabbleThreads {
	private static ScrabbleThreads scrabbleThreadsInstance = null;
	...
	private ScrabbleThreads(){
		
	}
	public static ScrabbleThreads getInstance(){
		synchronized (ScrabbleThreads.class) {
			if(scrabbleThreadsInstance == null){
				scrabbleThreadsInstance = new ScrabbleThreads();
				Collections.shuffle(scrabbleThreadsInstance.scrabbleLetterList);
			}					
		}		
		return scrabbleThreadsInstance;
	}
	...
	public LinkedList<String> getNLetters(int noOfLetters){
		...
		for(int i=0;i<=noOfLetters;i++){
			nLetters.add(scrabbleThreadsInstance.getScrabbleLetterList().remove(0));
		}
		return nLetters;
	}
}